# Site Copy — Harald Solaas

All translatable text, EN and ES side by side. Edit here, then update `src/dictionaries/en.ts` and `es.ts`.

---

## Metadata (SEO)

**Title**
- EN: Harald Solaas — Technology that serves people
- ES: Harald Solaas — Tecnología al servicio de las personas

**Description**
- EN: Senior Software Engineer & Technology Consultant. From AI solutions to full-stack products, I build technology that serves people — and communities that outlive me.
- ES: Ingeniero de Software Senior y Consultor Tecnológico. Desde soluciones de IA hasta productos full-stack, construyo tecnología al servicio de las personas — y comunidades que me trasciendan.

**OG Title**
- EN: Harald Solaas — Technology that serves people
- ES: Harald Solaas — Tecnología al servicio de las personas

**OG Description**
- EN: Senior Software Engineer & Technology Consultant. From AI solutions to full-stack products, building technology that serves people and communities that outlive me.
- ES: Ingeniero de Software Senior y Consultor Tecnológico. Desde soluciones de IA hasta productos full-stack, construyo tecnología al servicio de las personas y comunidades que me trasciendan.

**Keywords**
- EN: Harald Solaas, Hari Solaas, Senior Software Engineer, Technology Consultant, AI Solutions, AI Consultant, React, Next.js, Full Stack Developer
- ES: Harald Solaas, Hari Solaas, Ingeniero de Software Senior, Consultor Tecnológico, Soluciones de IA, Consultor de IA, React, Next.js, Desarrollador Full Stack

---

## Navigation

| Key | EN | ES |
|-----|----|----|
| Brand | Hari | Hari |
| Values | Values | Valores |
| Now | Now | Ahora |
| Story | Story | Historia |
| Contact | Contact | Contacto |
| Toggle menu | Toggle menu | Abrir menú |

---

## Hero

**Name**
- EN: Harald Solaas
- ES: Harald Solaas

**Tagline**
- EN: I started meditating at 15, teaching at 16, and writing code at 20. I haven't stopped doing any of them.
- ES: Empecé a meditar a los 15, a enseñar a los 16 y a programar a los 20. No dejé de hacer ninguna.

**Scroll CTA**
- EN: Scroll to meet me
- ES: Scrolleá para conocerme

---

## Value 1: "Outlive"

### Statement
- EN: I build things that outlive me.
- ES: Construyo cosas que me trascienden.

### Proof: Community
**Title**
- EN: 50 volunteers, 500+ lives touched
- ES: 50 voluntarios, 500+ vidas impactadas

**Description**
- EN: Led 50 volunteers as part of a team to teach 500+ people in a Buenos Aires slum. Built the volunteer infrastructure, then stepped away. The program continues to this day without him.
- ES: Lideré, junto a un equipo, a 50 voluntarios para enseñar a más de 500 personas en una villa de Buenos Aires. Construí la infraestructura de voluntarios y después migré al próximo desafío. El programa continúa hasta el día de hoy sin mí.

### Proof: Personal
**Title**
- EN: A basketball team from nothing
- ES: Un equipo de básquet desde cero

**Description**
- EN: Created a basketball team at age 29 because he was too old to join one. Built it from nothing, played for two years, then left due to injury. The team kept playing for two more years.
- ES: Armé un equipo de básquet a los 29 porque ya estaba grande para sumarme a un club. Lo armé de la nada, desarrollé al equipo, la logística, jugué dos años y me fui por una lesión. El equipo siguió compitiendo dos años más.

### Proof: Professional
**Title**
- EN: From 14 prod crashes to near-zero
- ES: De 14 explosiones de producción a casi cero

**Description**
- EN: At Cruise (autonomous vehicles, Silicon Valley), inherited a codebase with 20% test coverage and 14 crashes/month. Left it at 70%+ coverage with 0.3 crashes/month. The testing infrastructure he built became the team's foundation.
- ES: En Cruise (vehículos autónomos, Silicon Valley), heredé una base de código con 20% de cobertura de tests y 14 caídas por mes. Lo dejé en 70%+ de cobertura con 0.3 incidentes por mes. La infraestructura de testing que construí se convirtió en la base del equipo.

---

## Value 2: "Humans"

### Statement
- EN: I see the humans behind the system.
- ES: Veo a las personas detrás del sistema.

### Proof: Origin
**Title**
- EN: A psychologist's son
- ES: Hijo de un psicólogo

**Description**
- EN: Father was a psychologist and organizational consultant. From childhood, Hari learned to see systems as collections of people — their motivations, rewards, and sense of belonging.
- ES: Mi padre era psicólogo y consultor organizacional. Desde chico aprendí a ver los sistemas como conjuntos de personas — sus motivaciones, recompensas y sentido de pertenencia.

### Proof: Formative
**Title**
- EN: Youngest trauma relief teacher in Latin America
- ES: El profesor de alivio del trauma más joven de Latinoamérica

**Description**
- EN: At 15, became the youngest Art of Living trauma relief teacher in Latin America. Worked with people who lost homes in Villa La Angostura. Worked with orphans, rehab youth, and past criminals. Learned that behind every difficult exterior, there's a story and there's love.
- ES: A los 15, me convertí en el profesor de alivio de trauma más joven de El Arte de Vivir en Latinoamérica. Trabajé con personas que perdieron sus hogares en Villa La Angostura. Trabajé con huérfanos, jóvenes en rehabilitación y en procesos judiciales. Aprendí que detrás de cada exterior difícil, hay una historia y hay amor.

### Proof: Professional
**Title**
- EN: The bridge between engineering and people
- ES: El puente entre la ingeniería y las personas

**Description**
- EN: At Carewell, rebuilt a healthcare platform so exhausted family caregivers could find what they need faster. At Estée Lauder, a React component library serving 200+ ecommerce sites wasn't about code — it was about the person trying to buy a product. At every company, ended up as the bridge between engineering and product — because he sees the humans, not just the code.
- ES: En Carewell, reconstruí una plataforma de salud para que familiares agotados que cuidan de adultos mayores puedan encontrar lo que necesitan más rápido. En Estée Lauder, una librería de componentes React sirviendo 200+ sitios de ecommerce no se trataba de código — se trataba de la persona del otro lado intentando comprar un producto. En cada empresa, terminé siendo el puente entre ingeniería y producto — porque veo a las personas, no solo el código.

---

## Value 3: "Percentile"

### Statement
- EN: I go to the last percentile.
- ES: Voy por el último 1%.

### Proof: Cruise
**Title**
- EN: Autonomous vehicle analytics
- ES: Analítica de vehículos autónomos

**Description**
- EN: Built the analytics platform used by executives and operations staff across the company. Reduced execution time by 90% on critical components.
- ES: Construí la plataforma de analítica usada por ejecutivos y personal de operaciones de toda la empresa. Reduje el tiempo de ejecución un 90% en componentes críticos.

**Metrics**

| Value | EN | ES |
|-------|----|----|
| 20→70% | Test coverage in 3 months | Cobertura de tests en 3 meses |
| 14→0.3 | Crashes per month | Caídas por mes |
| 90% | Execution time reduction | Reducción tiempo de ejecución |

### Proof: Carewell
**Title**
- EN: $21M healthcare platform rebuild
- ES: Reconstrucción de plataforma de salud de $21M

**Description**
- EN: I was a key member of a team of four engineers that rebuilt a $21M healthcare ecommerce platform using React, Next.js, Tailwind CSS, and GraphQL. The site loads instantly, works without JavaScript, and achieves exceptional SEO scores. It launched on time, with increased engagement and sales.
- ES: Fui parte clave de un equipo de 4 ingenieros que reconstruyó una plataforma de ecommerce de salud de USD $21M con React, Next.js, Tailwind CSS y GraphQL. El sitio carga instantáneamente, funciona sin JavaScript y logra puntajes SEO excepcionales. Se lanzó a tiempo, con mayor engagement y ventas.

**Metrics**

| Value | EN | ES |
|-------|----|----|
| $21M | Platform rebuilt | Plataforma reconstruída |
| 100 | Lighthouse performance | Performance en Lighthouse |

### Proof: Estée Lauder
**Title**
- EN: 200+ brand sites globally
- ES: 200+ sitios de marcas en todo el mundo

**Description**
- EN: Built flexible, reusable components serving 200+ brand sites globally. Reduced time to first interaction. Multi-tenancy at the extreme — every deployment matters when hundreds of thousands of users depend on it daily.
- ES: Construí componentes flexibles y reutilizables sirviendo 200+ sitios de marcas a nivel global. Reduje el tiempo a primera interacción. Multi-tenancy al extremo — cada deploy importa cuando cientos de miles de usuarios dependen de él diariamente.

**Metrics**

| Value | EN | ES |
|-------|----|----|
| 200+ | Sites served | Sitios servidos |
| 60→85% | Test coverage | Cobertura de tests |

### Quote
- EN: "Not leaving anything for later. Going to the one percentile of making the user experience awesome. Optimizing as much as it makes sense, as much as it adds value."
- ES: "No dejar nada para después. Ir hasta el último 1% de hacer que la experiencia del usuario sea increíble. Optimizar tanto como tenga sentido, tanto como agregue valor."

---

## Value 4: "Serve"

### Statement
- EN: Technology should serve people, not replace them.
- ES: La tecnología debe servir a las personas, no reemplazarlas.

### Proof: Art of Living
**Title**
- EN: Streaming platform for wellbeing
- ES: Plataforma de streaming para bienestar

**Description**
- EN: Built a streaming platform for the Art of Living Foundation with thousands of monthly users, using gamified video challenges to deepen engagement with meditation and wellbeing content.
- ES: Hice una plataforma de streaming para la Fundación El Arte de Vivir con miles de usuarios mensuales, usando desafíos de video con gamification para mejorar el compromiso con contenido de meditación y bienestar.

### Proof: Cruise
**Title**
- EN: Tools that build the future
- ES: Herramientas que construyen el futuro

**Description**
- EN: Built tools that helped car engineers, testers, and data scientists visualize hundreds of thousands of data points from autonomous drives. Building the tools that build the future of transportation.
- ES: Construí herramientas que ayudaron a ingenieros de autos, testers y científicos de datos a visualizar cientos de miles de puntos de datos de rutas de vehículos automatizados (si, autos robot... si, sin conductor). Construyendo las herramientas que hacen al futuro del transporte.

### Proof: Carewell
**Title**
- EN: Technology for vulnerable moments
- ES: Tecnología para momentos vulnerables

**Description**
- EN: Making healthcare supplies accessible to families caring for aging loved ones. Technology that reduces friction in a moment of vulnerability.
- ES: Hacer accesibles los suministros de salud para familias que cuidan a seres queridos mayores. Tecnología que reduce la fricción en un momento de vulnerabilidad.

### Proof: Nubi
**Title**
- EN: Financial access for 50,000+
- ES: Acceso financiero para 50.000+

**Description**
- EN: Gave 50,000+ Latin Americans easier access to their PayPal funds — international transfers, withdrawals, credit card management.
- ES: Dimos a más de 50.000 latinoamericanos acceso más fácil a sus fondos de PayPal — transferencias internacionales, retiros, gestión de tarjetas de crédito.

### Proof: Philosophy
**Title**
- EN: Inspired by Muhammad Yunus
- ES: Inspirado por Muhammad Yunus

**Description**
- EN: Inspired by the "Banker to the Poor" — the idea that systems (financial, technological, organizational) can be designed to serve people who have been forgotten.
- ES: Inspirado por el «Banquero de los Pobres» — la idea de que los sistemas (financieros, tecnológicos, organizacionales) pueden diseñarse para servir a personas que han sido olvidadas.

### Quote
- EN: "I want to make technology your ally, your friend — not a threat."
- ES: "Quiero hacer de la tecnología tu aliada, tu amiga — no una amenaza."

---

## Value 5: "Joy"

### Statement
- EN: I bring joy to the work.
- ES: Traigo diversión al trabajo.

### Proof: Basketball
**Title**
- EN: Too old to join, so he built one
- ES: Muy grande para unirme, así que armé uno

**Description**
- EN: Too old to join a team at 29, so he created one. Joined an amateur league. It became its own living thing.
- ES: Muy grande para meterme a un equipo a los 29, así que creé uno. Me sumé a una liga amateur. Se convirtió en algo con vida propia.

### Proof: Teaching
**Title**
- EN: Three years in a slum
- ES: Tres años en una villa

**Description**
- EN: Teaching teenagers breathing techniques, human values, how to be kids again. Helping adolescents leave crime. Showing that there's another way.
- ES: Enseñando a adolescentes técnicas de respiración, valores humanos, cómo volver a ser chicos. Ayudándolos a salir del crimen. Mostrando que hay otro camino.

### Proof: Art of Living
**Title**
- EN: Teacher since 2022
- ES: Profesor desde 2022

**Description**
- EN: Teaching courses, building community on social media, learning to sell with authenticity, networking, building volunteer groups rooted in ethics.
- ES: Enseñando cursos, construyendo comunidad en redes sociales, aprendiendo a vender con autenticidad, haciendo networking, construyendo grupos de voluntarios basados en ética.

### Proof: Daily practice
**Title**
- EN: 9 years of daily practice
- ES: 9 años de práctica diaria

**Description**
- EN: 9 years of daily Sudarshan Kriya breathing practice and meditation. Joy isn't accidental — it's a discipline.
- ES: 9 años de práctica diaria de Sudarshan Kriya y meditación. La alegría no es accidental — es una disciplina.

### Quote
- EN: "Having fun while we do the things that need to be done. Celebrating our victories, learning from our defeats and keeping our chin up."
- ES: "Divertirse mientras hacemos las cosas que hay que hacer. Celebrar nuestras victorias, aprender de nuestras derrotas y mantener la frente en alto."

---

## Now Section

**Heading**
- EN: What I'm Building Right Now
- ES: Lo que estoy armando ahora

**Subheading**
- EN: I'm always building something — in code, in community, or in myself. Here's what I'm working on these days.
- ES: Siempre estoy construyendo algo — en código, en comunidad o en mí mismo. Esto es en lo que estoy trabajando estos días.

### Card 1: Art of Living Workshops [Teaching / Enseñanza]
**Title**
- EN: Art of Living Workshops
- ES: Talleres de El Arte de Vivir

**Description**
- EN: Next dates for meditation and breathing technique courses. Learn Sudarshan Kriya and tools for managing stress, anxiety, and emotional wellbeing.
- ES: Próximas fechas para cursos de meditación y técnicas de respiración. Aprendé Sudarshan Kriya y herramientas para manejar el estrés, la ansiedad y el bienestar emocional.

**Status:** EN: Upcoming / ES: Próximamente
**CTA:** EN: See Upcoming Dates / ES: Ver próximas fechas

### Card 2: Tech & AI Partner [Building / Desarrollo]
**Title**
- EN: Technology & AI Partner for Businesses
- ES: Socio tecnológico y de IA para negocios

**Description**
- EN: Helping businesses bring their operations into the digital world — from AI-powered automation and intelligent workflows to full product builds. If you need a technology partner who understands both the tech and the business, let's talk.
- ES: Ayudando a empresas a llevar sus operaciones al mundo digital — desde automatización con IA y flujos de trabajo inteligentes hasta productos completos. Si necesitás un socio tecnológico que entienda tanto la tecnología como el negocio, hablemos.

**Status:** EN: Ongoing / ES: Sucediendo ahora
**CTA:** EN: Get in Touch / ES: Contactame

### Card 2b: AI Solutions [Building / Desarrollo]
**Title**
- EN: AI Solutions That Serve People
- ES: Soluciones de IA al servicio de las personas

**Description**
- EN: Building AI-powered tools that genuinely help — not replace — the people who use them. From intelligent automation to custom AI integrations, always with the human at the center.
- ES: Construyendo herramientas con IA que genuinamente ayudan — no reemplazan — a las personas que las usan. Desde automatización inteligente hasta integraciones de IA a medida, siempre con el humano en el centro.

**Status:** EN: Ongoing / ES: Sucediendo ahora
**CTA:** EN: Get in Touch / ES: Contactame

### Card 3: Tree Planting [Community / Comunidad]
**Title**
- EN: Tree Planting Initiative
- ES: Iniciativa de plantación de árboles

**Description**
- EN: Annual birthday tradition — planting trees with the community. Building something that will outlast all of us.
- ES: Tradición anual de cumpleaños — plantar árboles con la comunidad. Construyendo algo que nos va a trascender a todos.

**Status:** EN: Annual / ES: Anual
**CTA:** EN: Learn More / ES: Saber más

### Card 4: Youth Breathwork [Teaching / Enseñanza]
**Title**
- EN: Breathwork & Leadership for Youth
- ES: Respiración y liderazgo para jóvenes

**Description**
- EN: Ongoing programs with university students and young professionals, combining breathwork, meditation, and practical tools for emotional strength, purpose, and conscious leadership.
- ES: Programas en curso con estudiantes universitarios y jóvenes profesionales, combinando respiración, meditación y herramientas prácticas para fortalecer la resiliencia emocional, el propósito y el liderazgo consciente.

**Status:** EN: Ongoing / ES: Sucediendo ahora
**CTA:** EN: Volunteer / ES: Ser voluntario

### Card 5: Surf [Personal]
**Title**
- EN: Follow My Adventures
- ES: Seguí mis aventuras

**Description**
- EN: Keeping adventure, art, travel and physical challenges as part of my everyday life. Follow along on Instagram.
- ES: Mantengo la aventura, el arte y los desafíos físicos como parte de mi día a día. Seguime en Instagram.

**Status:** EN: In Progress / ES: En progreso
**CTA:** EN: Follow on Instagram / ES: Seguime en Instagram

---

## Timeline

**Heading**
- EN: The Full Story
- ES: La historia completa

**Subheading**
- EN: A compact arc from age 15 to now.
- ES: Un arco compacto desde los 15 hasta hoy.

**Expand button**
- EN: Expand Timeline / ES: Ver la línea de tiempo

**Collapse button**
- EN: Collapse Timeline / ES: Ocultar la línea de tiempo

**Tech heading**
- EN: Technologies / ES: Tecnologías

### Entries

**2009**
- EN: Art of Living — Age 15 | Began meditation practice and training as a trauma relief teacher. Youngest Art of Living teacher in Latin America.
- ES: Art of Living — 15 años | Comencé la práctica de meditación y la formación como profesor de alivio de trauma. El profesor más joven de El Arte de Vivir en Latinoamérica.

**2011–2012**
- EN: Villa La Angostura | Post-disaster community support — working with people who lost their homes.
- ES: Villa La Angostura | Apoyo comunitario post-desastre — trabajando con personas que perdieron sus hogares.

**2011–2012**
- EN: Service | Enseñar técnicas de manejo emocional a orphans, at-risk youth, and past criminals.
- ES: Servicio | Enseñé técnicas de manejo emocional a huérfanos, jóvenes en rehabilitación y procesos judiciales.

**2014–2017**
- EN: University of Belgrano | BSc in Business Management. Started programming at 20 out of necessity for entrepreneurship contests.
- ES: Universidad de Belgrano | Licenciatura en Administración de Empresas. Empecé a programar a los 20 por necesidad en concursos de emprendedurismo.

**2016–Present**
- EN: Independent Software Engineer — building and scaling real-world systems for organizations in health, legal services, and public-facing institutions. From complex claim platforms and verification flows to internal tools and production-grade frontends. Where engineering became ownership, judgment, and long-term responsibility. The beginning of a lifelong craft.
- ES: ngeniero de Software Independiente — diseñando y construyendo sistemas reales para organizaciones de salud, servicios legales e instituciones con impacto público. Desde plataformas complejas de reclamos y flujos de verificación hasta herramientas internas y frontends en producción. El momento en que la ingeniería pasó a ser hacerse cargo, decidir bien y pensar a largo plazo. El comienzo de un oficio de por vida.

**2016–2022**
- EN: Community Service | Slum programs with 50 volunteers and 500+ participants. 3-year soccer club youth program. Volunteer team building.
- ES: Servicio comunitario | Programas en villas con 50 voluntarios y 500+ participantes. Programa juvenil de fútbol de 3 años. Creación de equipos de voluntarios.

**2018–2019**
- EN: GuruDevelopers | Ecommerce, Art of Living streaming platform, WordPress plugins. First "real" software factory experience.
- ES: GuruDevelopers | Ecommerce, plataforma de streaming de El Arte de Vivir, plugins de WordPress. Primera experiencia en una «fábrica de software» real.

**2019–2020**
- EN: Litebox — Technical Lead | Nubi digital wallet (50K+ users), BI startup, marine apps, mobile game (50K+ downloads), personalized dog nutrition ecommerce. Mentored juniors to mid-level.
- ES: Litebox — Líder Técnico | Billetera digital Nubi (50K+ usuarios), startup de BI, apps marinas, juego móvil (50K+ descargas), ecommerce de nutrición canina personalizada. Mentoré juniors a nivel mid.

**2020–2021**
- EN: The Estée Lauder Companies | Senior Frontend Engineer — React component library serving 200+ ecommerce sites. Test coverage 60%→85%. Global multi-tenancy at scale.
- ES: The Estée Lauder Companies | Ingeniero Frontend Senior — Librería de componentes React sirviendo 200+ sitios de ecommerce. Cobertura de tests 60%→85%. Multi-tenancy global a escala.

**2021–2023**
- EN: Cruise via Toptal | Senior Software Engineer — Autonomous vehicle analytics platform. Testing 20%→70%, crashes 14→0.3/month. D3.js data visualization. First Silicon Valley company.
- ES: Cruise vía Toptal | Ingeniero de Software Senior — Plataforma de analítica de vehículos autónomos. Testing 20%→70%, caídas 14→0.3/mes. Visualización de datos con D3.js. Primera empresa de Silicon Valley.

**2022**
- EN: Art of Living Teacher | Became a certified teacher for core Art of Living programs. Teaching courses, building community.
- ES: Profesor de El Arte de Vivir | Me certifiqué como profesor de los programas principales de El Arte de Vivir. Enseñando cursos, construyendo comunidad.

**2023**
- EN: Norway | Reconnecting with heritage and rebuilding family bridges.
- ES: Noruega | Reconectando con mis ancestros y reconstruyendo puentes en la familia.

**2023–Present**
- EN: Senior Consultant | Carewell ($21M healthcare ecommerce rebuild), Colgate (technology advisor), AI-powered solutions, various clients. Evolution from engineer to trusted advisor.
- ES: Consultor Senior | Carewell (reconstrucción de ecommerce de salud de $21M), Colgate (asesor tecnológico), soluciones con IA, varios clientes. Evolución de ingeniero a asesor de confianza.

---

## Contact

**Heading**
- EN: Let's build something.
- ES: Construyamos algo.

**Description**
- EN: I work as a senior technology consultant — helping companies build the right products with the right technologies, including AI solutions that genuinely serve your team and your users. I bring deep engineering expertise, human communication, and a service-oriented mindset. If you want an engineer who cares about your problem as much as you do, let's talk.
- ES: Trabajo como consultor tecnológico senior — ayudando a empresas a construir los productos correctos con las tecnologías correctas, incluyendo soluciones de IA que genuinamente sirvan a tu equipo y a tus usuarios. Traigo experiencia profunda en ingeniería, comunicación humana y una mentalidad de servicio. Si querés un ingeniero que se preocupe por tu problema tanto como vos, hablemos.

**Case study link text**
- EN: Read the Carewell Case Study on Toptal
- ES: Leer el caso de estudio de Carewell en Toptal

**Link labels** (Email, LinkedIn, Toptal, Instagram — same in both languages)

---

## Footer

**Copyright**
- EN: Harald Solaas — harisolaas.com
- ES: Harald Solaas — harisolaas.com
