...
import type { SinergiaParrafoDict } from "@/dictionaries/types";
...
export default function SinergiaParrafoLanding({ dict, locale }: Props) {
  const otherLocale = locale === "es" ? "en" : "es";
  const localeLabel = locale === "es" ? "EN" : "ES";

  const [remaining, setRemaining] = useState<number>(
    sinergiaParrafoConfig.capacity,
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/sinergia-parrafo/availability`)
      .then((r) => r.json())
      .then((d) => {
        if (d?.ok) {
  ...
  const handleCheckout = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await fetch("/api/sinergia-parrafo/checkout", {
        method: "POST",
  ...
  const capacityStr = String(sinergiaParrafoConfig.capacity);
  ...
  const ctaCopy = dict.rsvp.cta.replace(
    "{amount}",
    sinergiaParrafoConfig.ticketPriceLabel,
  );
  const priceLine = dict.rsvp.priceLine.replace(
    "{amount}",
    sinergiaParrafoConfig.ticketPriceLabel,
  );
  ...
  return (
    <div className="sinergia-theme">
      <a
        href={`/${otherLocale}/sinergia-parrafo`}
      ...
      <main className="overflow-hidden bg-cream text-charcoal">
            ...
            <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2">
              <div className="rounded-2xl border border-sage/20 bg-white/70 p-6">
                <h3 className="font-serif text-2xl text-forest">
                  {dict.hosts.sinergia.name}
                </h3>
                <p className="mt-2 text-sm leading-relaxed text-charcoal/75 md:text-base">
                  {dict.hosts.sinergia.role}
                </p>
              </div>
              <div className="rounded-2xl border border-sage/20 bg-white/70 p-6">
                <h3 className="font-serif text-2xl text-forest">
                  {dict.hosts.parrafo.name}
                </h3>
                <p className="mt-2 text-sm leading-relaxed text-charcoal/75 md:text-base">
                  {dict.hosts.parrafo.role}
                </p>
              </div>
            </div>
        ...
        {/* ───── PRACTICAL ───── */}
        <Section id="practical" className="px-6 py-16 md:py-24">
          <div className="mx-auto max-w-2xl">
            <h2 className="font-serif text-3xl text-forest md:text-4xl">
            ...
            <ul className="mt-10 space-y-5">
              {dict.practical.items.map((item, i) => (
                <motion.li
                  key={item.title}
        ...
        <Section id="rsvp" className="bg-cream-dark/40 px-6 py-16 md:py-24">
          <div className="mx-auto max-w-lg">
            ...
            <p className="mt-2 text-center text-xs font-semibold uppercase tracking-wider text-terracotta">
              {priceLine}
            </p>
              ...
              <AnimatePresence mode="wait">
                {!isFull ? (
                  ...
                  >
                    ...
                    >
                      {submitting ? dict.rsvp.submitting : ctaCopy}
                    </button>
