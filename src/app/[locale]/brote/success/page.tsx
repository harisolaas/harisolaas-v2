  ...
  return (
    ...
    >
      <div
        ...
        style={{
          background:
            "radial-gradient(80% 70% at 50% 120%, rgba(196,112,75,0.3) 0%, rgba(196,112,75,0) 60%)",
        }}
      />
      <div className="relative z-10 mx-auto max-w-md">
        <p className="text-6xl">🌳</p>
        <h1 className="mt-6 font-serif text-4xl text-cream [text-shadow:0_0_40px_rgba(196,112,75,0.3)]">
          {t.heading}
        </h1>
        <p className="mt-4 text-lg leading-relaxed text-cream/70">{t.body}</p>

        <div className="mt-8 rounded-xl border border-cream/10 bg-cream/5 p-5 backdrop-blur-sm">
          <p className="text-sm leading-relaxed text-cream/60">{t.emailNote}</p>
        </div>

        <div className="mt-6">
          <p className="text-sm text-cream/40">{t.noEmail}</p>
          <a
            href={whatsappUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="mt-2 inline-flex items-center gap-2 rounded-full border border-cream/25 px-5 py-2.5 text-sm font-medium text-cream transition-colors hover:border-cream/50 hover:bg-cream/10"
          >
