...
export async function generateMetadata({
  ...
  return {
    title: dict.sinergiaParrafo.failurePage.title,
    robots: { index: false, follow: false },
  };
}

export default async function SinergiaParrafoFailurePage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const dict = await getDictionary(locale as Locale);
  const copy = dict.sinergiaParrafo.failurePage;
      ...
      <div className="sinergia-theme min-h-[100svh] bg-cream">
        <main className="mx-auto flex min-h-[100svh] max-w-lg flex-col items-center justify-center gap-8 px-6 py-16 text-center">
          ...
          <div className="flex flex-col items-stretch gap-3 md:flex-row">
            <Link
              href={`/${locale}/sinergia-parrafo#rsvp`}
