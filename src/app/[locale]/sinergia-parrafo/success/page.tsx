...
export async function generateMetadata({
  ...
  return {
    title: dict.sinergiaParrafo.successPage.title,
    robots: { index: false, follow: false },
  };
}

export default async function SinergiaParrafoSuccessPage({
  params,
}: {
  params: Promise<{ locale: string }>;
}) {
  const { locale } = await params;
  const dict = await getDictionary(locale as Locale);
  const copy = dict.sinergiaParrafo.successPage;
  const addressLine = copy.addressLine
    .replace("{venue}", sinergiaParrafoConfig.venueName)
    .replace("{address}", sinergiaParrafoConfig.exactAddress);
