...
const DISCOUNT_CODE = "CAMINOALACIMA";
...
export async function POST(req: Request) {
  try {
    ...
    if (action === "validate") {
      ...
      const valid = code.trim().toUpperCase() === DISCOUNT_CODE;
      if (!valid) {
    ...
    if (action === "checkout") {
      ...
      const preference = await new Preference(mp).create({
        body: {
          items: [
            {
              id: "brote-ticket-cima",
              title: "BROTE — Entrada Comunidad CIMA",
              quantity: 1,
              unit_price: broteConfig.cimaPriceRaw,
          ...
          metadata: { type: "ticket", source: "cima" },
        },
      });
